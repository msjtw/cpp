...
## źródło
```c++
// zrodlo: http://informatykaplus.edu.pl/upload/list/czytelnia/Struktury_danych_i_ich_zastosowania.pdf
```
...
void insert(node drz_prz[], int p, int q, int w){
    p += base ;
    q += base ;
    drz_prz[p].wart = max(w, drz_prz[p].wart);
    drz_prz[p].max = max(w, drz_prz[p].max);
    if(p != q){
        drz_prz[q].max = max(w, drz_prz[q].max);
        drz_prz[q].wart = max(w, drz_prz[q].wart);
    }
    while(q > 0){
        if(p/2 != q/2){
            if(p%2 == 0){
                drz_prz[p+1].wart = max(w, drz_prz[p+1].wart);
                drz_prz[p+1].max = max(w, drz_prz[p+1].max);
            }
            if(q%2 == 1){
                drz_prz[q-1].wart = max(w, drz_prz[q-1].wart);
                drz_prz[q-1].max = max(w, drz_prz[q-1].max);
            }
        }
        if(p < base ){
            drz_prz[p].max = max(drz_prz[2*p].max, drz_prz[2*p+1].max);
            drz_prz[q].max = max(drz_prz[2*q].max, drz_prz[2*q+1].max);
        }
        p >>= 1;
        q >>= 1;
    }
...
int query(node drz_prz[], int p, int q){
    ...
    q += base ;
    while(q > 0){
        ret = max(ret, max(drz_prz[p].wart, drz_prz[q].wart));
        if(p/2 != q/2){
            if(p%2 == 0){
                ret = max(ret, drz_prz[p+1].max);
            }
            if(q%2 == 1){
                ret = max(ret, drz_prz[q-1].max);
            }
        }
        p >>= 1;
        q >>= 1;
